...
const Theater = ({ theaterId, seminari, selectedDate, filterSeminario, setSelectedDate }) => {
	const {
		register,
		handleSubmit,
		setValue,
	...
	const onAddShowtime = async (data) => {
		try {
			...
			if (!data.seminario) {
				toast.error('Please select a seminario', {
					position: 'top-center',
					autoClose: 2000,
					pauseOnHover: false
				})
			...
			const response = await axios.post(
				'/showtime',
				{ seminario: data.seminario, showtime, theater: theater._id, repeat: data.repeat, isRelease: data.isRelease },
				{
					headers: {
						Authorization: `Bearer ${auth.token}`
					}
					...
					<>
						...
						{filterSeminario?.name && (
							<div className="mx-4 flex gap-2 rounded-md bg-gradient-to-r from-indigo-600 to-blue-500 p-2 text-white">
								...
								{`You are viewing the showtimes of "${filterSeminario?.name}"`}
							</div>
						)}
					</>
				)}
