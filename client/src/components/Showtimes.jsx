			...
			{seminari?.map((seminario, index) => {
				return (
					...
					sortedShowtimes[seminario._id] && (
						<div key={index} className="flex items-center shadow-md">
							{showSeminarioDetail && <img src={seminario.img} className="w-32 px-4" />}
											...
											<button
												...
												className={
													isPast(new Date(showtime.showtime))
														? `flex items-center gap-1 px-2 py-1 text-lg bg-gray-200 border border-black text-black ${
														...
														? 'flex items-center gap-1 bg-blue-900 px-2 py-1 text-lg text-white'
